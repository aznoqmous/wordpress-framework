...
class KernelEventsSubscriber implements EventSubscriberInterface
{
    ...
    {
        if (empty($_SERVER['REQUEST_METHOD'] ?? '')) $_SERVER['REQUEST_METHOD'] = $request->getMethod();

        if (empty($_SERVER['REQUEST_URI'] ?? '')) $_SERVER['REQUEST_URI'] = $request->getRequestUri();

        if (empty($_SERVER['PHP_SELF'] ?? '')) $_SERVER['PHP_SELF'] = $request->getScriptName();

        if (empty($_SERVER['PATH_INFO'] ?? '')) $_SERVER['PATH_INFO'] = $request->getPathInfo();
    }
